...
export const reorderItems = createAsyncThunk(
  "lists/reorderItems",
  async ({ groupId, listId, itemIds }) => {
    const response = await fetchWithAuth(
      `${NEXT_PUBLIC_API_URL}/groups/${groupId}/lists/${listId}/items/reorder`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(reorderItems.pending, (state, action) => {
        state.loading = true;
        state.error = null;
        const { listId, itemIds } = action.meta.arg;
        const list = state.items.find((l) => l._id === listId);
        if (list) {
          const itemsById = new Map(list.items.map((i) => [i._id, i]));
          list.items = itemIds
