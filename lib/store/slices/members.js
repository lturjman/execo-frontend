...
export const fetchMember = createAsyncThunk(
  ...
  async ({ groupId, member }) => {
    const response = await fetchWithAuth(
      `${NEXT_PUBLIC_API_URL}/groups/${groupId}/members/${member._id}`
    )
...
export const fetchMembers = createAsyncThunk(
  ...
  async ({ groupId }) => {
    const response = await fetchWithAuth(
      `${NEXT_PUBLIC_API_URL}/groups/${groupId}/members`
    )
...
export const updateMember = createAsyncThunk(
  ...
  async ({ groupId, member }) => {
    ...
    if (!response.ok) { throw new Error('Erreur lors de la modification du membre') }
    const responseBody = await response.json()
    return responseBody.data
  }
)
