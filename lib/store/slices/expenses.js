...
export const fetchExpense = createAsyncThunk(
  ...
  async ({ groupId, expense }) => {
    const response = await fetchWithAuth(
      `${NEXT_PUBLIC_API_URL}/groups/${groupId}/expenses/${expense._id}`
    )
    if (!response.ok) { throw new Error('Erreur lors de la récupération de la dépense') }
...
export const fetchExpenses = createAsyncThunk(
  ...
  async ({ groupId }) => {
    const response = await fetchWithAuth(
      `${NEXT_PUBLIC_API_URL}/groups/${groupId}/expenses`
    )
...
export const createExpense = createAsyncThunk(
  ...
  async ({ groupId, expense }) => {
    ...
    if (!response.ok) { throw new Error('Erreur lors de la création de la expense') }
    const responseBody = await response.json()
    return responseBody.data
  }
)
...
export const updateExpense = createAsyncThunk(
  ...
  async ({ groupId, expense }) => {
    ...
    if (!response.ok) { throw new Error('Erreur lors de la modification de la dépense') }
    const responseBody = await response.json()
    return responseBody.data
  }
)
