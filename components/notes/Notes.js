...
import { ChevronDownIcon, ChevronUpIcon, EllipsisVerticalIcon } from '@heroicons/react/24/solid'
...
function writeStoredSeenNoteIds (ids) {
  try {
    window.localStorage.setItem(SEEN_NOTE_IDS_KEY, JSON.stringify(ids.slice(-1000)))
  } catch (error) {
    return null
  }
}
            ...
            {notes.length === 0
              ...
              : (
                <div className='flex flex-col items-center gap-2'>
                  {notes.map((note) => {
                    const isMine = currentMember && note.member === currentMember._id
                    const author = members.find((member) => member._id === note.member)
                    ...
                    return (
                      ...
                      >
                        <div className='flex items-center justify-between gap-2 mb-1'>
                          <p className={`text-sm font-bold truncate ${color.name}`}>
                          ...
                          {isMine && !isEditing && (
                            ...
                            >
                              <EllipsisVerticalIcon className={`size-5 ${color.icon}`} />
                            </button>
                          )}
                        </div>
                        ...
                        {isMine && menuNoteId === note._id && (
                          <NoteRemove
                            ...
                            note={note}
                            onEdit={startEdit}
                            onMenuClose={() => setMenuNoteId(null)}
                          />
                        )}
